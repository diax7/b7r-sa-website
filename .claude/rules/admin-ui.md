...
14. The sidebar shows no document count (ADR-058). A number that asks for action is a badge:
   ...
   the same number, one number per entry: never one badge carrying two entries' numbers
   (ADR-062 amended); its tone, red or amber, never grey; its entry) and its sentence in
