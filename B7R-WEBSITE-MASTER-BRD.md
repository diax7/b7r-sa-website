...
Amended 2026-09-13 (Phase 2a, ADR-030): tag-based revalidation is not used. Every public page and the metadata routes carry `revalidate = 60` (ISR), the data layer reads Payload directly with per-render deduplication, and the `afterChange` / `afterDelete` hooks call `revalidatePath` on the static routes that render the changed content (home, listing, sitemap; the pages a global feeds). Product detail pages are left to the timer: Next 16's file-system cache answers 404 for a `dynamicParams = false` route after an on-demand revalidation. A publish is on the listing at once and on the product page within a minute; draft autosaves change nothing. The IndexNow ping and the CDN purge stay planned for 2b.
