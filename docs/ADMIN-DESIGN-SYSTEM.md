...
| Inbox (a section of Site) | `modules/inbox/*`, `app/api/contact/route.ts`, `app/api/inbox/*` | Admins and editors. One row per contact form submission (`messages`), written by the contact route before the notification e-mail leaves and never created by hand (ADR-061); the sender's fields as read-only lines that no update rewrites, the status (three pills) and the internal notes the two fields anyone writes; three actions above the form (`MessageActions`: "Reply on WhatsApp" as a `wa.me` link with a greeting in the sender's language, "Reply by e-mail" as a `mailto:` with a subject in it, "Mark handled" as an `ApiAction` on `/api/inbox/messages/:id/handle`, which turns the status select without dirtying the form); the red badge on Messages and the dashboard's Inbox card read one number. An admin deletes; nothing deletes itself. |
| Bookings (the inbox section's second entry) | `modules/bookings/*`, `app/api/bookings/*`, `app/(site)/book/*` | Admins and editors. One row per consultation a merchant booked on `/book` or the contact card (`bookings`), written by the booking routes and the sweep, never created by hand (ADR-062); the merchant's fields an admin's to correct, the status (four pills: booked green, rescheduled amber, cancelled red, completed neutral) and the notes an editor's; the facts the routes write (the Meet link, the calendar state, the reminders sent) as read-only lines; one action above the form (`BookingActions`: "Remind on WhatsApp", a `wa.me` link with the reminder in the merchant's language, while the booking is ahead). The inbox badge adds today's bookings to the new messages; the Inbox card lists the next three. The `Booking` global (Site) holds the switch, the grid and the calendar owner; the Google Calendar connection's Test reads the host's free/busy for today. |
