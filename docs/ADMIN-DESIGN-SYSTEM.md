...
- Localised fields (ADR-043): the panel's locale control switches every localised field
  ...
  pill in its controls, one line that never wraps ("Editing the English content. Fields
  marked EN are per language; the rest is shared with Arabic."; on a phone it takes its own
  line under the buttons); register it through `admin/document/config.ts` on every
