...
2. **Two languages, two axes.** The panel reads in English or in Arabic, chosen per person
   with the switch at the trailing end of the header (Payload's `switchLanguage` and its
   `payload-lng` cookie; ADR-056, which reversed the English-only panel of 2026-09-13, and
   its 2026-09-19 amendment for the switch). Every string of ours exists in both
...
- Current registry: products `Shirt`, pages `FileText`, faqs `CircleHelp`, testimonials
  ...
  inbox section `Inbox` (ADR-061), the engine section `Bot`; the dashboard entry
  `LayoutDashboard` (a place, like a global's).
...
The panel speaks English or Arabic; the person chooses with the switch at the trailing end
of the header (or at the foot of the phone drawer, or the account view's select; all
Payload's `switchLanguage`, kept in its `payload-lng` cookie) and Payload sets `lang` and
`dir` on the document. Everything of ours follows the request's language, never the build's.
...
| Piece | File | Notes |
|---|---|---|
| Sidebar | `modules/cms/admin/nav/*` | One tree, one breakpoint (ADR-058). `nav-client.tsx` is the aside (Payload's outer `nav` classes and its `useNav` state, so the template's grid follows) with the brand row, the tree, the rail and the foot; `tree.tsx` the tree; `rail.tsx` the rail; `groups.ts` the data, `badges.ts` the badges, `keyboard.ts` and `active.ts` the pure rules. **The tree**, top to bottom: the brand row (the logo, a link to the dashboard); the dashboard as an entry; the five task groups (Site · Catalogue · Blog · Visibility · Admin, ADR-046), each a 40 px row (weight 600, an 8 px dot in the group's hue before the name, the chevron at the end; the whole row toggles, `button[aria-expanded]` owning a `role="group"` labelled by it) over its entries: 36 px with a 24 px disc in the group's hue, secondary entries (hubs, authors, tags under Posts; the checklist, snapshots, prompts and citations under the Score page; counts under Traffic) 32 px and 13 px under a 2 px guide line; a **section** is a sub-heading inside a group with its own icon and its entries as secondary ones, placed first or last in the group (`NAV_SECTIONS[key].place`, one order for the tree, the flyout, the keyboard and the active row through `nav/order.ts`): the inbox opens Site (messages, then bookings; ADR-061), the content engine closes Blog; a hairline between groups, never between entries. The active entry sits on its group's tint with weight 500, a 3 px bar on the leading edge and a solid disc, `aria-current="page"`, its group forced open (a click closes it for that page only). Hover is `surface-2`, focus the accent ring. No counts; a badge only where a number asks for action (`badges.ts`, red or amber; the runs', the drafts' and the inbox's numbers and sentences are the dashboard's, ADR-059, ADR-061). **The state** (open or collapsed, each group) lives in Payload's `nav` preference, written whole, keyed by the group's registry key. **Above 1024 px** (Payload's `m`) the sidebar is inline: open at 264 px, or the 64 px **rail** when collapsed by the one button above the account (« open, » collapsed, mirrored in RTL): the brand mark, the dashboard icon, the five group icons (the active group's carries the bar, a group with a badge a dot) and the avatar; a click on a group opens a 224 px flyout with the group's entries (a `DropdownMenu` of links, not modal, opening away from the rail, focus moved in, Esc back; its active entry wears the hue, weight 500 and the bar, no tint). Both the tree and the rail are in the markup and `admin.css` shows one by the aside's open class (`[data-admin-tree]`, `[data-admin-rail-list]`, `[data-rail-hide]`, `[data-rail-center]`), so a collapsed sidebar paints as a rail on the first frame; hydration adds `data-admin-rail`, the tooltips and the flyouts. Payload closes the nav under its `l` breakpoint (1440 px); a layout effect puts the preference back, so 1025 to 1440 px is inline like any desktop. **At 1024 px and under** it is a drawer over the page (320 px, the full width under 768): the header's hamburger opens it, the X at the same spot in the drawer, Esc, a tap on the scrim or a navigation closes it; the page behind is `inert` while it is open; rows are 44 px; the collapse button hides; the account block and the panel's language switch (`language-switch.tsx` with `placement="drawer"`, the same control as the header's) sit at the foot. |
| Header actions | `modules/cms/admin/header/actions*`, `header/control.tsx` | The hamburger (`[data-admin-menu]`, at the leading edge of the header at 1024 px and under, hidden above), a bordered 240 px search box that opens the palette (the Ctrl K hint; 320 px on focus), a bordered "View website" link with text, and the panel's language switch last, at the trailing end (below); all fold to icons at the drawer widths, where each carries its tooltip beside its `aria-label` (`IconTooltip` and the shared `control` class live in `control.tsx`). Payload's account avatar and its two hamburgers are hidden in `@layer payload` (our account block and our hamburger own them); so is the content locale switcher and its spacer (ADR-057: no locale switch), whose gutter the controls took. The row never spills: Payload's crumbs shrink first, the last one truncating with its ellipsis, while the controls keep their width, so a long title on a 390 px phone (or at 1025 px with the controls at full width) never pushes the controls past the edge (`admin.css`, the header block). |
| Language switch | `modules/cms/admin/nav/language-switch.tsx`, `document/form-modified.tsx` | One control, two placements (ADR-056, amended 2026-09-19): `placement="header"` at the trailing end of the header's actions (top right in English, top left in Arabic), `placement="drawer"` at the foot of the phone drawer. Above 1024 px a bordered `role="group"` ("Panel language") of the two names, each in its own language and never translated («العربية», "English", from Payload's `languageOptions`), the current one `aria-pressed` on `surface-2` at weight 500; at 1024 px and under, in the header, one icon whose tooltip and label say the other language ("Switch to Arabic"). A click is Payload's own `switchLanguage` (the `payload-lng` cookie, then `router.refresh()`; `<html lang dir>` follow), the control disabled while it is pending. Since the refresh reloads a document form from the server, the `FormModifiedSentinel` (first in every config's `beforeDocumentControls`, through `document/config.ts`) marks `<body data-admin-form-modified>` while a form holds unsaved changes, and the switch then asks in a `Dialog` ("Unsaved changes are lost when the language changes. Save first, or switch anyway.": "Switch anyway" amber, "Cancel" bordered). It changes the UI language only, never the content locale. |
...
| Dashboard | `modules/cms/admin/dashboard/*` | Eight sections top to bottom (ADR-059, ADR-061), one server render, the reads in parallel and each guarded (a failed reader shows its section with the "not available" word, never a blank page). (1) The greeting by the Riyadh hour (name in the accent), the 7 / 30 / 90 day range as links at the trailing edge (`?days=`, server-rendered, no client state; `rules.ts`) and the "needs a hand" line (failed runs this week, a connection at its limit, an enabled connection whose last test failed, documents without English, drafts older than a week; each a link, or one sentence). (2) Four tiles, each one link (`tiles.tsx`, `tile-data.ts`): visits with the change against the previous range (`trafficSummary` twice; with a `umami` snapshot in the range the tile is Umami's visitors, the landings on the line under, ADR-048 amended), the cited rate on the category prompts over 28 days with the engines' count, the score with its trend, "went live" in the range with the drafts waiting. (3) Where visits come from (`traffic/admin/traffic-card.tsx`): the bars by group, the top three entry pages, the crawler reads, a people row (Umami's visitors, page views and average visit for the range, through yesterday) when a snapshot exists, "All traffic" into the same range. (4) What the assistants say (`visibility/admin/assistants-card.tsx`): per engine the cited and linked rates, the last run, the next run computed from the prompts' periods (`schedule.ts`, `duePrompts`) or the plain sentence; no "Run now" (it costs money). (5) Inbox (`inbox/admin/inbox-card.tsx`, before the content, a third of the row beside it on a desktop): the sentence with the count of new messages linked to the list filtered on New, the newest three with the sender, the inquiry and the first words, each a link to its form, "All messages"; «لا رسائل جديدة» when there is nothing; the same number and sentence as the sidebar's badge on Messages, and its bookings line the badge on Bookings. (6) Content (`content-card.tsx`): the home tile with when it was published, one row per content collection with published, drafts (linked to the list filtered on `_status`) and missing English (linked to the first English form), the last five saves by people (`data.ts`: the content groups only, no machine rows, no untitled unsaved draft), "Write a post" and "Add a product" as bordered buttons by permission. (7) Engine and spend (`ai-content/admin/engine-card.tsx`): the state, posts against the monthly cap and today's cost against the daily cap as bars (violet, amber from 80 %, red at the cap), the next slot, one row per AI connection with its spend against its limit (amber with "No monthly limit" when it has none, red at the limit), runs and last test. (8) Server (`server-card.tsx`): a `details` collapsed by default and open when a row is red, the ten health rows (`healthReport()`), the version, the jobs queue as the next run of each scheduled task on the Riyadh clock, "Full report". An editor sees the sections they may open; a reader that needs an admin is skipped. One hue per card (ADR-060): the section's title icon takes its group's hue (`DashboardSection` `hue`: visits and assistants pink, inbox and content blue, engine violet, server slate; a tile its one disc) and the body stays neutral, the content card's entity discs and buttons included; a draft in the saves reads amber. Numbers, dates and moments through `admin/format.ts`; the hooks are `data-admin-dashboard-<section>`. Every in-admin link is Payload's `Link`: no reload. |
...
| Inbox (a section of Site) | `modules/inbox/*`, `app/api/contact/route.ts`, `app/api/inbox/*` | Admins and editors. One row per contact form submission (`messages`), written by the contact route before the notification e-mail leaves and never created by hand (ADR-061); the sender's fields as read-only lines that no update rewrites, the status (three pills) and the internal notes the two fields anyone writes; three actions above the form (`MessageActions`: "Reply on WhatsApp" as a `wa.me` link with a greeting in the sender's language, "Reply by e-mail" as a `mailto:` with a subject in it, "Mark handled" as an `ApiAction` on `/api/inbox/messages/:id/handle`, which turns the status select without dirtying the form); the red badge on Messages counts the new messages, the same number and sentence as the dashboard's Inbox card first line (ADR-062 amended). An admin deletes; nothing deletes itself. |
| Bookings (the inbox section's second entry) | `modules/bookings/*`, `app/api/bookings/*`, `app/(site)/book/*` | Admins and editors. One row per consultation a merchant booked on `/book` or the contact card (`bookings`), written by the booking routes and the sweep, never created by hand (ADR-062); the merchant's fields an admin's to correct, the status (four pills: booked green, rescheduled amber, cancelled red, completed neutral) and the notes an editor's; the facts the routes write (the Meet link, the calendar state, the reminders sent) as read-only lines; one action above the form (`BookingActions`: "Remind on WhatsApp", a `wa.me` link with the reminder in the merchant's language, while the booking is ahead). A red badge on Bookings counts today's bookings still ahead, the Inbox card's second line (ADR-062 amended); the card lists the next three. The `Booking` global (Site) holds the switch, the grid and the calendar owner; the Google Calendar connection's Test reads the host's free/busy for today. |
