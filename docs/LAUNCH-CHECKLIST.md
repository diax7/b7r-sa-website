...
| 20 | S3 bucket `b7r-media` with public read; the six `S3_*` values set in the app's environment | Dhia | open | `/api/health` → `media: s3`; SeaweedFS locally |
...
- GitHub repository `diax7/b7r-sa-website` exists, **private since 2026-09-24** (Dhia's call; Actions minutes are metered from then); the platform project is still Dhia's.
...
## Added 2026-09-24 (spec 010, the Appearance global; ADR-064, photo delivery)

| # | Item | Owner | Status | Notes |
|---|---|---|---|---|
| 48 | The Appearance global saved on production with Sea mist named in both languages, and the booking blurb in both | *code* | done 2026-09-24 | Through the admin API, the two writes `content:migrate --force` would make: the production database port does not answer from outside CranL, and the full script would have re-created the FAQ «كيف أربط متجري؟», which production holds as «كيف يتم الربط؟» (a duplicate on `/faq`) |
| 49 | The traced logo compared with the PNG in the header at 1440 and 390, and Sea mist with `resources/gradient/reference.png` (its deep bloom lightened from #00609b to #4f9cd7 so text reads on it) | Dhia | done 2026-09-25: both approved (Dhia) | By eye; ADR-065 phase 1c and 1d |
| 50 | IBM Plex Sans Arabic is under the OFL with the Reserved Font Name "Plex", and the served subsets keep the family name: rename the subset's family, or keep it | Dhia | done 2026-09-25: kept as it is (Dhia) | `scripts/subset-fonts.sh`, `public/fonts/*-OFL.txt` |
| 51 | The renditions on production (ADR-064): generated by the build of a99f079; a `-1080.avif` and its `.webp` answer 200 with `cdn-cache: HIT` on the second request; no public page requests `/_next/image` | *code* | done 2026-09-24 | A read-only check of the live site passed 84 of 84 (every sitemap URL 200, the drawn icons and sprite, no console error or broken image, axe on 14 pages at 1440 and 390); an upload of a 3000 by 2250 photo takes 32 s (RUNBOOK "Assets") |
| 52 | CranL builds `main` from the now private repository | Dhia | open until a deploy proves it | CranL's GitHub webhook answers 401 on every delivery, yet it deployed each merge of 2026-09-24; if a merge stops deploying, reconnect GitHub in CranL with access to `b7r-sa-website` |
