...
| 4 | Resend domain `b7r.sa` verified (SPF, DKIM, DMARC); test contact email received at contact@b7r.sa | Dhia | open | `RESEND_API_KEY`, `RESEND_FROM`, `RESEND_SEGMENT_ID`, the contact e-mail in Site settings → Contact; `/api/health` → `contact: live`, `newsletter: live` |
