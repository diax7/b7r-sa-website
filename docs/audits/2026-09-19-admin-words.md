...
| Descriptions over 140 characters | 29 before, 0 after; `CAP_EXCEPTIONS` is empty |
...
| Glossary rows | 186 (31 Latin-kept terms, 232 refused forms) |
...
| New gates | `tests/admin-glossary.test.ts` (146 tests); the description rule in `tests/admin-config.test.ts`: the 140 cap, the label-noun check, the storage-word check |
...
«الخدمة», a limit is «الحد», a switch is «مفعّل» / «معطّل», the desktop is «الحاسوب»).

## Ten before and after

| Where | Before (en) | After (en) | Before (ar) | After (ar) |
|---|---|---|---|---|
| Products, Name | The product's name on its card, its page title, the designer's picker and llms.txt. | On the card, the page title, the designer's picker and llms.txt. | اسم المنتج كما يظهر في البطاقة، وعنوان صفحته، وقائمة المصمّم، وملف llms.txt. | في البطاقة، وعنوان صفحته، وقائمة المصمّم، وملف llms.txt. |
...
- **Validation messages and `Refused` reasons**: read; short, verb-first, in both languages
  already; the glossary gate does not reach inside a validator's closure, which is the one
  gap the next pass could close by lifting them into the maps.
