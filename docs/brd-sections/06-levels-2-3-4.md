...
*Amended 2026-09-19 (Level 4 PR 4c, ADR-048 amended): Umami's visitors and page views are no longer "if ever". A Connection of the kind "Umami" holds the Cloud API key (the website id stays in Site settings → Analytics; a self-hosted Umami names its address on the row); the nightly job calls `stats` once per day on Riyadh day boundaries and keeps one `metrics` row per day (`source: umami`: visitors, page views, visits, bounces, total time), 90 days back on the first run, then the last two complete days each night. The dashboard's visits tile shows Umami's visitors with the site's own landings under it, and the "Where visits come from" card gains a people row (visitors, page views, average visit) for the same 7, 30 or 90 days; without a row both read as before. No share URL embed, no events, no top pages from Umami (the counter and Search Console cover them); GA4 stays in GA.*
