...
test.describe('hero (BRD 6.4.1)', () => {
  ...
  // photo is fetched at its own width, never an upscaled candidate (ADR-051).
  for (const [width, path] of [
    [2560, '/'],
    [3440, '/en'],
  ] as const) {
    test(`at ${width} px the hero is 1920 px wide, centred, and the photo is not upscaled (${path})`, async ({
      browser,
      baseURL,
    }) => {
      ...
      expect(box.x).toBe((width - 1920) / 2);
      expect(box.y).toBe(0);
