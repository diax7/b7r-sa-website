...
function umamiPayload(options: {
  ...
  return { payload, written, updated };
}
...
describe("the pull's Umami days (ADR-048 amended)", () => {
  ...
  it('writes one metrics row per day read, names a failure, and skips without the connection', async () => {
    ...
    try {
      ...
      // The first run: 90 calls, 90 rows.
  ...
  it('stops at the day that fails and keeps the days before it', async () => {
    let n = 0;
    const fetcher = (async () => {
      n += 1;
      return n === 2 ? Response.json({}, { status: 500 }) : Response.json(RECORDED_STATS);
    }) as typeof fetch;
    vi.stubGlobal('fetch', fetcher);
    try {
...
/** A Payload whose `metrics` rows are the given `umami` days, recording the query. */
function metricsPayload(rows: Array<{ date: string; data: Partial<PeopleSummary> }>) {
  const asked: unknown[] = [];
  const payload = {
    find: async ({ where }: { where: unknown }) => {
      asked.push(where);
      const since = /"greater_than_equal":"([^"]+)"/.exec(JSON.stringify(where))?.[1] ?? '';
      return { docs: rows.filter((r) => r.date >= since) };
    },
  } as unknown as Payload;
  return { payload, asked };
}

/** A pulled day whose other numbers follow its visitors. */
const day = (date: string, visitors: number) => ({
  date,
  data: { visitors, pageviews: visitors * 3, visits: visitors + 1, bounces: 1, totaltime: 600 },
});

describe("the dashboard's people reader (ADR-048 amended)", () => {
  it('sums the umami rows of the range and counts the days that have one', async () => {
  ...
  it('reads no row as zero days, so the tile and the card stay as they are without Umami', async () => {
    expect(await peopleSummary(metricsPayload([]).payload, { days: 7, now: NOW })).toEqual({
      visitors: 0,
      pageviews: 0,
      visits: 0,
      bounces: 0,
...
const people = (visitors: number, days: number): PeopleSummary => ({
  visitors,
...
describe('the visits tile with Umami (ADR-048 amended)', () => {
  ...
  it('reads as before without a row: the landings and their change', () => {
    ...
    expect(
      dashboardTiles({ ...base, people: { current: people(0, 0), double: people(0, 0) } })[0],
    ).toMatchObject({ label: 'Visits', value: '4,881' });
  });

  it("shows Umami's visitors as the number, the landings under, and the visitors' own change", () => {
    const tile = dashboardTiles({
      ...base,
      people: { current: people(3900, 7), double: people(6900, 14) },
    })[0]!;
    ...
    // A fresh connection: the previous range has no row, so no change is claimed.
    expect(
      dashboardTiles({ ...base, people: { current: people(3900, 7), double: people(3900, 7) } })[0],
    ).toMatchObject({ value: '3,900', detail: '4,881 landings' });
    // The previous range has rows that sum to nothing: the "none before" line, as the landings say.
    expect(
      dashboardTiles({ ...base, people: { current: people(12, 7), double: people(12, 14) } })[0],
    ).toMatchObject({ detail: '4,881 landings · none in the previous 7 days' });
    // One landing is singular; a failed traffic reader leaves the landings out.
    expect(
      dashboardTiles({
        ...base,
        traffic: { current: summary(1), double: summary(1) },
        people: { current: people(2, 7), double: people(2, 7) },
      })[0],
    ).toMatchObject({ detail: '1 landing' });
    expect(
      dashboardTiles({
        ...base,
        traffic: null,
    ...
    ).toMatchObject({ label: 'Visitors', value: '2', detail: '' });
  });
